...
export default function AddressParserTestPage() {
  ...
  const addField = (key: FieldKey) => {
    ...
    const newAvailable = ALL_FIELDS.filter(k => ![...fields, {
      key,
      value: ""
    }].some(f => f.key === k));
  ...
  const handleSplit = async () => {
    ...
    const phoneMatch = cleanedInput.match(/(?:\+7|\+49|8)?[\s-]?\(?\d{2,4}\)?[\s-]?\d{3,4}[\s-]?\d{2,4}[\s-]?\d{2,4}/);
    ...
    if (phoneMatch) {
      ...
      cleanedInput = cleanedInput.replace(phoneMatch[0], "");
    }
    if (emailMatch) {
      detected.push({
        key: "email",
      ...
      cleanedInput = cleanedInput.replace(emailMatch[0], "");
    }
    ...
    try {
      ...
      if (s.name) newFields.push({
        key: "name",
        value: capitalizeAllWords(addSpacesBetweenWords(transliterate(s.name)))
      });
      if (s.street) newFields.push({
        key: "street",
        value: capitalizeAllWords(addSpacesBetweenWords(transliterate(s.street)))
      });
      if (s.house_number) newFields.push({
        key: "house_number",
        value: addSpacesBetweenWords(transliterate(s.house_number))
      });
      if (s.block) newFields.push({
        key: "block",
        value: addSpacesBetweenWords(transliterate(s.block))
      });
      if (s.kv) newFields.push({
        key: "kv",
        value: addSpacesBetweenWords(transliterate(s.kv))
      });
      if (s.city) newFields.push({
        key: "city",
        value: capitalizeAllWords(addSpacesBetweenWords(transliterate(s.city)))
      });
      if (s.postal_code) newFields.push({
        key: "postal_code",
        value: transliterate(s.postal_code)
      });
      if (s.country) newFields.push({
        key: "country",
        value: capitalizeAllWords(transliterate(s.country))
      });
    } catch (e) {
  ...
  return <div className="max-w-4xl mx-auto space-y-6 py-0 px-[16px]">
      ...
      {visible && <Card className="bg-muted/40 mt-6">
          <CardContent className="p-4 space-y-4">
            <h2 className="text-lg font-semibold mb-2">{t("consumer_section_assignment")}</h2>
            {fields.map((f, i) => <div key={i} className="flex gap-3 items-center">
                <Select value={f.key} onValueChange={val => changeKey(i, val as FieldKey)}>
                  <SelectTrigger className="w-[200px]">
                    <SelectValue>{t(`consumer_${f.key}`)}</SelectValue>
                  </SelectTrigger>
                  <SelectContent>
                    {ALL_FIELDS.filter(key => !fields.some(x => x.key === key && x.key !== f.key)).map(key => <SelectItem key={key} value={key}>
                        {t(`consumer_${key}`)}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
            ...
            {/* Add & Save */}
            {availableFields.length > 0 && <div className="flex items-center gap-2 pt-4">
                <Select value={fieldToAdd ?? undefined} onValueChange={val => setFieldToAdd(val as FieldKey)}>
                  ...
                  <SelectContent>
                    {availableFields.map(key => <SelectItem key={key} value={key}>
                        {t(`consumer_${key}`)}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
