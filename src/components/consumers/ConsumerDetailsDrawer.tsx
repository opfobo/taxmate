...
const ConsumerDetailsDrawer = ({ 
  ...
  // Fetch recent orders for this consumer
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 gap-4">
                <div className="space-y-1">
                  <h3 className="text-sm font-medium text-muted-foreground">{t("contact_information")}</h3>
                ...
                <div className="space-y-1">
                  <h3 className="text-sm font-medium text-muted-foreground">{t("address_information")}</h3>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <p className="text-sm font-medium">{t("address")}</p>
                      <p className="text-sm">{consumer.address_line1 || "-"}</p>
                      {consumer.address_line2 && <p className="text-sm">{consumer.address_line2}</p>}
                    </div>
                    <div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="flex flex-col items-center justify-center bg-muted p-3 rounded-lg">
                    ...
                    <p className="text-xl font-semibold">{formatCurrency(consumer.total_order_volume || 0)}</p>
                    <p className="text-xs text-muted-foreground">{t("total_volume")}</p>
                  </div>
                  <div className="flex flex-col items-center justify-center bg-muted p-3 rounded-lg">
                    <Clock className="h-4 w-4 mb-1 text-primary" />
                    <p className="text-xl font-semibold">{consumer.last_order_date ? format(new Date(consumer.last_order_date), "dd/MM/yy") : "-"}</p>
                  ...
                  <div className="space-y-2">
                    {recentOrders.map((order) => (
                      <div key={order.id} className="p-3 border rounded-md flex justify-between items-center">
                        <div>
                          <p className="text-sm font-medium">{order.order_number || order.id.substring(0, 8)}</p>
